...
const UA = "KeepSupplyProspector/2.5";
...
function parseLinks(html: string, ignoreHost: string[] = []): Hit[] { const out: Hit[] = []; const seen = new Set<string>(); const re = /<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi; let m: RegExpExecArray | null; while ((m = re.exec(html)) && out.length < 30) { const url = unwrap(m[1]); const title = clean(m[2]); if (!/^https?:/i.test(url) || title.length < 5) continue; if (ignoreHost.some((h) => url.includes(h))) continue; if (seen.has(url)) continue; seen.add(url); out.push({ title, url, snippet: "" }); } return out; }
function parseDdg(html: string) { return parseLinks(html, ["duckduckgo.com"]); }
function parseBing(html: string) { const out: Hit[] = []; for (const b of html.split(/<li[^>]+class=["'][^"']*b_algo[^"']*["'][^>]*>/i).slice(1)) { const m = b.match(/<h2[^>]*>\s*<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/i); if (!m) continue; const s = b.match(/<p[^>]*>([\s\S]*?)<\/p>/i); if (/^https?:/i.test(m[1])) out.push({ title: clean(m[2]), url: m[1], snippet: s ? clean(s[1]) : "", engine: "bing" }); if (out.length >= 20) break; } return out; }
function parseGoogle(html: string) { return parseLinks(html, ["google.com", "gstatic.com"]); }
function parseYahoo(html: string) { return parseLinks(html, ["yahoo.com"]); }
function parseStartpage(html: string) { return parseLinks(html, ["startpage.com", "google.com"]); }
function parseMojeek(html: string) { return parseLinks(html, ["mojeek.com"]); }
async function html(url: string, timeout = 2400) { const ctl = new AbortController(); const t = setTimeout(() => ctl.abort(), timeout); try { const r = await fetch(url, { headers: { "User-Agent": UA, "Accept-Language": "en-US,en;q=0.9" }, cache: "no-store", signal: ctl.signal }); return r.ok ? await r.text() : ""; } catch { return ""; } finally { clearTimeout(t); } }
async function engine(e: string, q: string, offset: number): Promise<Hit[]> {
  if (e === "ddg") return parseDdg(await html(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(q)}&s=${offset}`));
  if (e === "bing") return parseBing(await html(`https://www.bing.com/search?q=${encodeURIComponent(q)}&count=10&first=${offset + 1}`));
  if (e === "yahoo") return parseYahoo(await html(`https://search.yahoo.com/search?p=${encodeURIComponent(q)}&b=${offset + 1}`));
  if (e === "startpage") return parseStartpage(await html(`https://www.startpage.com/sp/search?query=${encodeURIComponent(q)}&cat=web`));
  if (e === "mojeek") return parseMojeek(await html(`https://www.mojeek.com/search?q=${encodeURIComponent(q)}&theme=light&autocomp=0`));
  return parseGoogle(await html(`https://www.google.com/search?q=${encodeURIComponent(q)}&start=${offset}`));
}
async function multiSearch(q: string, offset: number): Promise<Hit[]> { const results = await Promise.allSettled(["ddg", "bing", "yahoo", "google", "startpage", "mojeek"].map((e) => engine(e, q, offset))); return results.flatMap((r) => r.status === "fulfilled" ? r.value : []); }
...
function classify(h: Hit, hint: string): Prospect {
  ...
  const ammoHits = ["ammonia","anhydrous ammonia","rmp","psm","nh3"].filter((x) => c.includes(x)).length; const nonAmmo = ["co2 refrigeration","carbon dioxide refrigeration","glycol","hfc","hcfc"].some((x) => c.includes(x));
  ...
  const ammoniaLb = m ? Number((m[1] || m[2]).replace(/,/g, "")) : null; const state = stateGuess(`${h.title} ${h.snippet}`, hint); const city = cityGuess(`${h.title} ${h.snippet}`, state);
...
async function epaRmp(state: string): Promise<Prospect[]> {
  const sts = state === "All Western States" ? STATES : [state]; const urls = sts.map((s) => `https://ofmpub.epa.gov/frs_public2/frs_rest_services.get_facilities?state_abbr=${ABBR[s]}&pgm_sys_acrnm=RMP&program_output=yes&output=JSON`); const raws = await Promise.all(urls.map((u) => html(u, 5000))); const out: Prospect[] = [];
...
function queries(state: string, batch: number, sweep: number, selected: string[]) {
  const sts = state === "All Western States" ? STATES : [state]; const mode = MODES[sweep % MODES.length]; const pack = state === "All Western States" ? 3 : 1; const start = (batch * pack) % sts.length; const use = Array.from({ length: Math.min(pack, sts.length) }, (_, i) => sts[(start + i) % sts.length]); const aliases = selected.flatMap((x) => INDUSTRY_SEARCH_ALIASES[x] || [x]); const terms = (aliases.length ? aliases : mode[1]).slice(0, 24); const qs: string[] = [];
  for (const s of use) { for (let i = 0; i < Math.min(6, terms.length); i++) { const term = terms[(batch * 4 + sweep * 3 + i) % terms.length]; qs.push(`"${s}" ${term}`); qs.push(`"${s}" ${term} refrigeration`); } const cities = GEO[s] || []; if (cities.length) { const city = cities[(batch + sweep) % cities.length]; const term = terms[(batch + sweep) % terms.length]; qs.push(`"${city}" ${term}`); qs.push(`"${city}" ${term} permit`); qs.push(`"${city}" ${term} filetype:pdf`); } qs.push(`"${s}" ${terms[(batch + sweep + 5) % terms.length]} site:gov`); }
...
export async function POST(req: Request) {
  try {
    ...
    if (mode === "discover" || mode === "categoryDiscover") {
      const batch = Math.max(0, Number(b?.batch) || 0); const sweep = Math.max(0, Number(b?.sweep) || 0); const selected = Array.isArray(b?.selectedCategories) ? b.selectedCategories.filter((x: unknown): x is string => typeof x === "string") : [];
      ...
      return NextResponse.json({ mode: "discover", batch, totalBatches: cfg.total, sweep, searchMode: cfg.label, queryCount: cfg.qs.length, hitCount: hits.length, epaCount: epa.length, engines: ["DuckDuckGo", "Bing", "Yahoo", "Google", "Startpage", "Mojeek", "EPA RMP"], prospects: candidates, raw: `${cfg.label} pack ${batch + 1}/${cfg.total}: ${cfg.qs.length} query patterns across 6 public search engines + EPA RMP; ${candidates.length} new candidates.` });
    }
    const p = b?.prospect || {}; const name = p.name || "industrial refrigeration facility"; const city = p.city || ""; const qs = [`"${name}" ${city} industrial refrigeration`, `"${name}" ${city} ammonia refrigeration`, `"${name}" ${city} cold storage`, `"${name}" site:epa.gov ammonia`, `"${name}" site:gov refrigeration permit`, `"${name}" filetype:pdf refrigeration`]; const groups = await Promise.all(qs.map((q) => multiSearch(q, 0))); const hits = Array.from(new Map(groups.flat().map((h) => [h.url, h])).values()).slice(0, 50); return NextResponse.json({ mode: "research", dossier: hits.length ? [`LIVE PUBLIC WEB RESEARCH: ${name}`, "", ...hits.map((h, i) => `${i + 1}. ${h.title}\n${h.snippet}\nSource: ${h.url}`), "", "Industrial refrigeration is the primary target. Ammonia is optional; 10,000 lb applies only when ammonia is present."].join("\n") : `No public-search results were retrieved for ${name}.`, sources: hits.map((h) => h.url) });
